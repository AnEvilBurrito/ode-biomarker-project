#!/usr/bin/env python3
"""
Batch script for SVR hyperparameter benchmarking with fixed network-based feature selection
Focuses exclusively on testing different SVR configurations
"""
...
def create_svr_selection_pipeline(
    selection_method: callable, k: int, method_name: str, svr_config: Dict
):
    """
    Create pipeline for SVR model selection with fixed feature selection
    Includes hyperparameter tracking for SVR configurations
    """
    ...
    def pipeline_function(X_train: pd.DataFrame, y_train: pd.Series, rng: int):
        ...
        if no_features or len(selected_features) == 0:
            ...
            # Create SVR model with specific hyperparameters
            model = get_model_from_string("SVR", **svr_config)
        ...
        return {
            ...
            "svr_config": svr_config,  # Store SVR hyperparameters
            "scaler": scaler,
...
def model_selection_eval(
    ...
    *,
    ...
    svr_config = pipeline_components.get("svr_config", {})  # Get SVR config
    ...
    return {
        ...
        "svr_config": svr_config,  # Include SVR config in results
...
def main():
    """Main execution function for SVR hyperparameter benchmarking with fixed network-based feature selection"""
    # Initialize path loader and data link
    path_loader = PathLoader("data_config.env", "current_user.env")
    data_link = DataLink(path_loader, "data_codes.csv")
    
    # Setup experiment parameters - use same folder as benchmark_models.py
    ...
    # Create report file
    report_file = f"{file_save_path}svr_hyperparameter_benchmark_report_{exp_id}.md"
    ...
    with open(report_file, 'w', encoding='utf-8') as report:
        print_and_save("# SVR Hyperparameter Benchmarking Report (Fixed Network MRMR d3)", report)
        ...
        print_and_save("Starting SVR hyperparameter benchmarking with fixed network-based feature selection...", report)
    ...
    with open(report_file, 'a', encoding='utf-8') as report:
        ...
        print_and_save("## SVR Hyperparameter Configurations", report)
        # Define SVR configurations to test (12 total)
        svr_configs = [
            # Linear kernel variations (3 configurations)
            {"kernel": "linear", "C": 0.1},
            {"kernel": "linear", "C": 1},      # Current baseline
            {"kernel": "linear", "C": 10},
            
            # RBF kernel variations (6 configurations)
            ...
            # Polynomial kernel variations (2 configurations)
            {"kernel": "poly", "C": 1, "degree": 2},
            {"kernel": "poly", "C": 1, "degree": 3},
            
            # Sigmoid kernel variations (1 configuration)
            {"kernel": "sigmoid", "C": 1, "gamma": "scale"}
        ]
        
        print_and_save(f"Testing {len(svr_configs)} SVR configurations:", report)
        for i, config in enumerate(svr_configs, 1):
            config_str = ", ".join([f"{k}={v}" for k, v in config.items()])
            print_and_save(f"  {i}. SVR({config_str})", report)
        
        print_and_save("## Experiment Setup", report)
        # Setup experiment parameters
        feature_set_sizes = [100, 500]  # Two k values as specified
        ...
        print_and_save(f"Benchmarking {len(feature_selection_methods)} feature selection method across {len(feature_set_sizes)} feature sizes and {len(svr_configs)} SVR configurations", report)
        print_and_save(f"Total conditions: {len(feature_selection_methods) * len(feature_set_sizes) * len(svr_configs)}", report)
        print_and_save("Feature Selection Method: MRMR+Network (distance 3)", report)
        print_and_save("SVR Configurations: 12 total (3 linear, 6 RBF, 2 poly, 1 sigmoid)", report)
        ...
        # Register all conditions (method × size × SVR config combinations)
        ...
        for method_name, selection_method in feature_selection_methods.items():
            for k in feature_set_sizes:
                for svr_config in svr_configs:
                    # Create condition name that includes SVR hyperparameters
                    kernel = svr_config["kernel"]
                    c_val = svr_config["C"]
                    gamma_val = svr_config.get("gamma", "default")
                    if gamma_val == "scale":
                        gamma_str = "gammaScale"
                    elif gamma_val == "auto":
                        gamma_str = "gammaAuto"
                    else:
                        gamma_str = f"gamma{gamma_val}"
                    
                    degree_val = svr_config.get("degree", "default")
                    condition = f"SVR_{kernel}_C{c_val}_{gamma_str}_deg{degree_val}_k{k}_{method_name}"
                    
                    # Create pipeline for this method, size, and SVR config
                    pipeline_func = create_svr_selection_pipeline(selection_method, k, method_name, svr_config)
        ...
        print_and_save("Starting SVR hyperparameter benchmark...", report)
        ...
        # Extract hyperparameters and other info from condition for easier analysis
        df_benchmark["k_value"] = df_benchmark["condition"].str.extract(r'k(\d+)').astype(int)
        df_benchmark["svr_kernel"] = df_benchmark["condition"].str.split('_').str[1]
        df_benchmark["svr_C"] = df_benchmark["condition"].str.split('_').str[2].str.replace('C', '').astype(float)
        df_benchmark["svr_gamma"] = df_benchmark["condition"].str.split('_').str[3]
        df_benchmark["svr_degree"] = df_benchmark["condition"].str.split('_').str[4].str.replace('deg', '').replace('default', '3').astype(int)
        df_benchmark["method"] = df_benchmark["condition"].str.split('_').str[6]
        
        print_and_save("## Results", report)
        print_and_save("### Condition Breakdown", report)
        print_and_save("Condition breakdown:", report)
        print_and_save(df_benchmark[["condition", "svr_kernel", "svr_C", "svr_gamma", "svr_degree", "k_value", "method"]].head(10).to_string(), report)
        
        # Save results
        df_benchmark.to_pickle(f"{file_save_path}svr_hyperparameter_benchmark_{exp_id}.pkl")
        print_and_save(f"Results saved to: {file_save_path}svr_hyperparameter_benchmark_{exp_id}.pkl", report)
        ...
        # Show performance by kernel type
        kernel_summary = df_benchmark.groupby("svr_kernel")["model_performance"].agg(["mean", "std", "count"])
        print_and_save("\nPerformance by kernel type:", report)
        print_and_save(kernel_summary.round(4).to_string(), report)

        # Show performance by C value
        c_summary = df_benchmark.groupby("svr_C")["model_performance"].agg(["mean", "std", "count"])
        print_and_save("\nPerformance by C value:", report)
        print_and_save(c_summary.round(4).to_string(), report)
        ...
        # Time analysis by kernel
        time_summary = df_benchmark.groupby("svr_kernel")[["feature_selection_time", "model_training_time", "prediction_time"]].agg(["mean", "std", "count"])
        print_and_save("\nTime Analysis by Kernel:", report)
        ...
        print_and_save("## SVR Configuration Comparison", report)
        # Best performing configuration for each k value
        for k in feature_set_sizes:
            k_data = df_benchmark[df_benchmark["k_value"] == k]
            best_config_idx = k_data["model_performance"].idxmax()
            best_config = k_data.loc[best_config_idx]
            print_and_save(f"Best SVR configuration for k={k}: {best_config['condition']} (R² = {best_config['model_performance']:.4f})", report)
            print_and_save(f"  Kernel: {best_config['svr_kernel']}, C: {best_config['svr_C']}, Gamma: {best_config['svr_gamma']}, Degree: {best_config['svr_degree']}", report)
        ...
        print_and_save("SVR hyperparameter benchmarking with fixed network-based feature selection completed successfully!", report)
