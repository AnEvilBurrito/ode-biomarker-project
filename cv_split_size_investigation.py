#!/usr/bin/env python3
"""
Script to investigate the effect of cross-validation split size on Random Forest model (config 1)
Maintains the same robust repeated holdout validation approach as benchmark_models.py
Includes both k=100 and k=500 feature set sizes
"""
...
def create_model_selection_pipeline(
    ...
    def pipeline_function(X_train: pd.DataFrame, y_train: pd.Series, rng: int):
        ...
        if no_features or len(selected_features) == 0:
            ...
            # Use standard hyperparameters for RandomForestRegressor_config1
            if model_name == "RandomForestRegressor_config1":
...
def main():
    """Main execution function for CV split size investigation"""
    ...
    exp_id = "v1_rf_config1_k100_k500_split_size_investigation"
    ...
    # Create report file
    report_file = f"{file_save_path}cv_split_size_investigation_report_{exp_id}.md"
    ...
    with open(report_file, 'w', encoding='utf-8') as report:
        print_and_save("# Cross-Validation Split Size Investigation Report", report)
        ...
        print_and_save("Investigating the effect of cross-validation split size on RandomForestRegressor_config1 with k=100 and k=500...", report)
    ...
    with open(report_file, 'a', encoding='utf-8') as report:
        ...
        # Setup experiment parameters
        feature_set_sizes = [100, 500]  # Both k=100 and k=500 as requested
        split_sizes = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]  # 5% to 30% test size
        model_name = "RandomForestRegressor_config1"  # Only this model as requested
        ...
        print_and_save(f"Benchmarking {len(feature_selection_methods)} feature selection method across {len(feature_set_sizes)} feature sizes and {len(split_sizes)} split sizes", report)
        print_and_save(f"Total conditions: {len(feature_selection_methods) * len(feature_set_sizes) * len(split_sizes)}", report)
        print_and_save("Feature Selection Method: MRMR+Network (distance 3)", report)
        print_and_save(f"Model: {model_name}", report)
        print_and_save(f"Feature Set Sizes: {feature_set_sizes}", report)
        print_and_save(f"Split Sizes: {split_sizes}", report)
        ...
        for split_size in split_sizes:
            print_and_save(f"## Running experiments with split size: {split_size}", report)
            
            # Initialize Powerkit with current split size
            ...
            for method_name, selection_method in feature_selection_methods.items():
                for k in feature_set_sizes:
                    # Create condition name including split size
                    condition = f"{model_name}_k{k}_{method_name}_split{split_size}"
            ...
            print_and_save(f"Registered {len(pk.conditions)} conditions for split size {split_size} in {time.time() - start_time:.2f} seconds", report)
            
            print_and_save("## Benchmark Execution", report)
            # Run all conditions using Powerkit's parallel processing
            print_and_save(f"Starting model selection benchmark for split size {split_size}...", report)
            ...
            # Add split size information to results
            df_benchmark["split_size"] = split_size
            ...
            # Clear conditions for next split size
            pk.conditions = {}
        
        # Combine all results
        combined_results = pd.concat(all_results, ignore_index=True)
        
        print_and_save("## Combined Results", report)
        print_and_save(f"Total runs across all split sizes: {len(combined_results)}", report)
        print_and_save(f"Unique conditions: {combined_results['condition'].nunique()}", report)
        
        # Save combined results
        combined_results.to_pickle(f"{file_save_path}cv_split_size_investigation_{exp_id}.pkl")
        print_and_save(f"Results saved to: {file_save_path}cv_split_size_investigation_{exp_id}.pkl", report)
        
        print_and_save("### Performance Summary by Split Size", report)
        # Show performance by split size and k value
        performance_summary = combined_results.groupby(["split_size", "k_value"])["model_performance"].agg(["mean", "std", "count"])
        print_and_save("Performance by split size and k value:", report)
        print_and_save(performance_summary.round(4).to_string(), report)
        
        print_and_save("### Performance Summary by Split Size Only", report)
        # Show performance by split size (aggregated across k values)
        split_summary = combined_results.groupby("split_size")["model_performance"].agg(["mean", "std", "count"])
        print_and_save("Performance by split size:", report)
        print_and_save(split_summary.round(4).to_string(), report)
        
        print_and_save("### Performance Summary by k Value Only", report)
        # Show performance by k value (aggregated across split sizes)
        k_summary = combined_results.groupby("k_value")["model_performance"].agg(["mean", "std", "count"])
        print_and_save("Performance by k value:", report)
        print_and_save(k_summary.round(4).to_string(), report)
        
        print_and_save("### Time Analysis", report)
        print_and_save("Time Analysis by split size:", report)
        time_summary = combined_results.groupby("split_size")[["feature_selection_time", "model_training_time", "prediction_time"]].agg(["mean", "std", "count"])
        ...
        print_and_save(f"Average time per condition: {total_batch_time / len(combined_results):.2f} seconds", report)
        
        print_and_save("## Conclusion", report)
        print_and_save("Cross-validation split size investigation completed successfully!", report)
