...
def get_network_feature_size(nth_degree_neighbours: list, max_distance: int, X: pd.DataFrame = None) -> int:
    """
    Get the actual number of available network features for a given distance
    
    Args:
        nth_degree_neighbours: List of network features by distance
        max_distance: Distance to check (1-indexed)
        X: Optional DataFrame to filter features that exist in the data
        
    Returns:
        int: Number of available network features at the specified distance
    """
    ...
    # If no network features available, return 0
    if len(network_features) == 0:
        return 0
    
    # If X is provided, filter to features that exist in the data
    if X is not None:
        # Try to map network features to proteomics-compatible IDs
    ...
    return len(network_features)


def random_network_mrmr_select_wrapper(X: pd.DataFrame, y: pd.Series, k: int, 
                                      nth_degree_neighbours: list, max_distance: int) -> tuple:
    """
    Random Network + MRMR feature selection wrapper (control for network selection)
    First selects random features of the same size as network features at given distance,
    then applies MRMR to select k from random subset
    """
    # Step 1: Get network feature size for this distance
    network_size = get_network_feature_size(nth_degree_neighbours, max_distance, X)
    
    # If no network features available, return empty selection
    if network_size == 0:
        return [], np.array([])
    
    # Step 2: Select random features of the same size as network features
    random_features, _ = select_random_features(X, y, network_size)
    
    # Step 3: Apply MRMR to select k features from random subset
    # Cap k at available random features
    k_actual = min(k, len(random_features))
    
    # Get the random subset of features
    X_random = X[random_features]
    
    # Apply MRMR to random subset
    selected_features, scores = mrmr_select_fcq_fast(X_random, y, k_actual, verbose=0)
...
def main():
    ...
    with open(report_file, 'a', encoding='utf-8') as report:
        ...
        # Random Network + MRMR control methods (same feature space size as network methods)
        for distance in [2, 3, 4]:
            method_name = f"random_network_d{distance}"
            # Create partial function with network parameters using lambda with default argument
            feature_selection_methods[method_name] = lambda X, y, k, dist=distance: random_network_mrmr_select_wrapper(
                X, y, k, nth_degree_neighbours, dist
            )
        ...
        # Negative control (fixed-size random selection)
        ...
        print_and_save("Methods: MRMR+Network (d2, d3, d4), Random+Network (d2, d3, d4), MRMR, ANOVA-filter, and Random Selection (negative control)", report)
        ...
        print_and_save("## Network vs. Random Network Comparison", report)
        # Compare network-based selection vs. random network control
        print_and_save("### Performance Comparison: Network vs. Random Network", report)
        
        # Calculate actual network feature sizes for each distance
        network_sizes = {}
        for distance in [2, 3, 4]:
            network_size = get_network_feature_size(nth_degree_neighbours, distance, feature_data)
            network_sizes[distance] = network_size
            print_and_save(f"Distance {distance}: Network feature size = {network_size}", report)
        
        # Sanity check: Print random network feature sizes to verify they match network sizes
        print_and_save("\n### Sanity Check: Random Network Feature Sizes", report)
        print_and_save("Verifying that random network methods use the same feature space size as network methods:", report)
        
        for distance in [2, 3, 4]:
            # Test the random network wrapper to get the actual feature size used
            # Create a dummy test to get the feature size
            test_X = feature_data.iloc[:10]  # Small subset for testing
            test_y = label_data.iloc[:10]
            
            # Get the network size for this distance
            network_size = get_network_feature_size(nth_degree_neighbours, distance, feature_data)
            
            # Test the random network wrapper to see what size it uses
            try:
                # This will execute the first step of the random network wrapper
                random_features, _ = select_random_features(test_X, test_y, network_size)
                actual_random_size = len(random_features)
                
                print_and_save(f"Distance {distance}:", report)
                print_and_save(f"  Network feature size: {network_size}", report)
                print_and_save(f"  Random network feature size: {actual_random_size}", report)
                print_and_save(f"  Match: {'✓' if network_size == actual_random_size else '✗'}", report)
                
            except Exception as e:
                print_and_save(f"Distance {distance}: Error testing random network size - {e}", report)
        
        # Compare performance between network and random network methods
        print_and_save("\nPerformance Comparison (Network vs. Random Network):", report)
        
        for distance in [2, 3, 4]:
            network_method = f"mrmr_network_d{distance}"
            random_network_method = f"random_network_d{distance}"
            
            # Get performance for both methods
            network_perf = df_benchmark[df_benchmark["method"] == network_method]["model_performance"]
            random_network_perf = df_benchmark[df_benchmark["method"] == random_network_method]["model_performance"]
            
            if len(network_perf) > 0 and len(random_network_perf) > 0:
                network_mean = network_perf.mean()
                random_network_mean = random_network_perf.mean()
                performance_diff = network_mean - random_network_mean
                
                print_and_save(f"Distance {distance}:", report)
                print_and_save(f"  Network (MRMR+Network): {network_mean:.4f}", report)
                print_and_save(f"  Random Network (Random+MRMR): {random_network_mean:.4f}", report)
                print_and_save(f"  Difference (Network - Random): {performance_diff:.4f}", report)
                
                # Statistical test (t-test) if we have multiple runs
                if len(network_perf) > 1 and len(random_network_perf) > 1:
                    from scipy.stats import ttest_ind
                    t_stat, p_value = ttest_ind(network_perf, random_network_perf, nan_policy='omit')
                    print_and_save(f"  T-test: t={t_stat:.4f}, p={p_value:.4f}", report)
                else:
                    print_and_save("  (Single run - no statistical test available)", report)
            else:
                print_and_save(f"Distance {distance}: No data available for comparison", report)
        ...
        print_and_save("The experiment now includes proper random network controls that match the feature space size of network methods.", report)
        print_and_save("This allows for fair comparison between network-based selection and random selection with the same feature space size.", report)
