...
# %%
folder_name = "ThesisResult-FeatureImportanceRF-SHAP"
exp_id = "v1_rf_config1_shap_seeds20_batch4" 
...
# Define expected file patterns based on the file structure
file_patterns = [
    "dataset_type_analysis_v1_rf_config1_shap_seeds20_batch4.pkl",
    "feature_importance_analysis_v1_rf_config1_shap_seeds20_batch4.pkl",
    "shap_consensus_importance_signed_v1_rf_config1_shap_seeds20_batch4.pkl",
    "shap_consensus_importance_v1_rf_config1_shap_seeds20_batch4.pkl",
    "shap_iteration_importance_signed_v1_rf_config1_shap_seeds20_batch4.pkl",
    "shap_iteration_importance_v1_rf_config1_shap_seeds20_batch4.pkl"
]
...
for pattern in file_patterns:
    ...
    try:
        ...
        dataset_name = pattern.replace(f"_{exp_id}.pkl", "").replace("v1_rf_config1_shap_", "")
...
def create_positive_matrix_plot(signed_consensus_data, file_save_path, exp_id):
    ...
    dataset_types = ['dynamic', 'rnaseq', 'combined']
...
def create_cdk46_mirrored_barchart(signed_consensus_data, file_save_path, exp_id):
    ...
    # Create mirrored bar chart for RNA-seq features
    y_pos = np.arange(len(rnaseq_comparison_data))
    
    # Plot bars for RNA-seq-only (left) and combined (right)
    rnaseq_values = [data['rnaseq_only'] for data in rnaseq_comparison_data]
    combined_values = [data['combined'] for data in rnaseq_comparison_data]
    
    bars1 = ax2.barh(y_pos - bar_height/2, rnaseq_values, bar_height, 
                    color=positive_color, alpha=0.7, label='RNA-seq-Only', 
                    edgecolor='black', linewidth=1)
