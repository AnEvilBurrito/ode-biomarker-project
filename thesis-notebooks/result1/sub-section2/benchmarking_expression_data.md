...
if df_all is not None and "model_performance" in df_all.columns:
    df_all = df_all[
        (df_all["model_performance"] >= 0) & (df_all["model_performance"] <= 1)
    ]

if df_rna is not None and "model_performance" in df_rna.columns:
    df_rna = df_rna[
        (df_rna["model_performance"] >= 0) & (df_rna["model_performance"] <= 1)
    ]

if df_prot is not None and "model_performance" in df_prot.columns:
    df_prot = df_prot[
...
# Create robust heatmap with vmin=0, vmax=max to handle extreme values
plt.figure(figsize=(10, 7))

# Calculate robust vmax to handle extreme negative values gracefully
clean_data = heatmap_data.values[np.isfinite(heatmap_data.values)]
if len(clean_data) > 0:
    vmax = np.max(clean_data)
    # Ensure at least some range for visualization
    vmax = max(vmax, 0.1)
else:
    vmax = 0.5  # Default if no valid data
...
# Save the figure
os.makedirs(file_save_path, exist_ok=True)
fname = os.path.join(file_save_path, f"model_modality_stats_{exp_id}.png")
plt.savefig(fname, dpi=300, bbox_inches="tight")
print(f"Saved figure to: {fname}")
plt.show()
```
...
if "metrics" in df_all.columns:
    ...
    # Filter data to only include r2 values between 0 and 1
    if "r2" in plot_data.columns:
        plot_data = plot_data[(plot_data["r2"] >= 0) & (plot_data["r2"] <= 1)]
